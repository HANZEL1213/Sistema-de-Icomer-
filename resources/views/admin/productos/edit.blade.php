                        ...
                        <div class="card border-0 bg-light mb-3" id="descuentoPrincipalCard">
                            <div class="card-body">
                                ...
                                @php
                                    ...
                                    $ahorroPreview = $item->precio_descuento
                                        ? $item->precio - $item->precio_descuento
                                        : 0;
                                ...
                                <div id="descuentoCampos"
                                    ...
                                    <small class="text-muted d-block mt-3">
                                        Si activa la promoción debe indicar precio de descuento, fecha de inicio y fecha de
                                        finalización.
                                    </small>
                                </div>
                            </div>
                                    ...
                                    $item->variantes
                                        ->map(function ($variante) {
                                            return [
                                                ...
                                                'precio_descuento' =>
                                                    $variante->precio_descuento !== null
                                                        ? (int) $variante->precio_descuento
                                                        : '',
                                                'descuento_inicio' => $variante->descuento_inicio
                                                    ? $variante->descuento_inicio->format('Y-m-d\TH:i')
                                                    : '',
                                                'descuento_fin' => $variante->descuento_fin
                                                    ? $variante->descuento_fin->format('Y-m-d\TH:i')
                                                    : '',
                            ...
                            <div class="card-body">
                                ...
                                <div id="variantesCampos"
                                    ...
                                    <div class="border rounded bg-white p-3">
                                        ...
                                        <div id="variantesContainer" data-variantes='@json($variantesParaJs)'></div>
                                ...
                                <div id="relatedProductsContent" style="display: none; margin-top: 1.5rem;">
                                    ...
                                    <div class="row g-3" id="productosRelacionadosGrid">
                                        @forelse ($productosRelacionados as $productoRel)
                                            @php
                                                $varianteBase = $productoRel->usa_variantes
                                                    ? $productoRel->variantePrincipal ??
                                                        $productoRel->variantesActivas->first()
                                                    : null;

                                                if ($productoRel->usa_variantes && $varianteBase) {
                                                    $precioNormalRelacionado = (float) $varianteBase->precio;
                                                    $precioVentaRelacionado = (float) $varianteBase->precioVenta();
                                                    $stockRelacionado = (int) $varianteBase->stock_actual;
                                                    $tieneDescuento = $varianteBase->promocionVigente();
                                                } else {
                                                    $precioNormalRelacionado = (float) $productoRel->precio;
                                                    $precioVentaRelacionado = (float) $productoRel->precioVenta();
                                                    $stockRelacionado = (int) $productoRel->stock_actual;
                                                    $tieneDescuento = $productoRel->tienePromocionActiva();
                                                }

                                                $ahorroRelacionado = $tieneDescuento
                                                    ? max(0, $precioNormalRelacionado - $precioVentaRelacionado)
                                                    : 0;

                                                $porcentajeRelacionado =
                                                    $tieneDescuento && $precioNormalRelacionado > 0
                                                            ...
                                                            <div class="flex-grow-1 overflow-hidden">
                                                                ...
                                                                @if ($productoRel->usa_variantes && $varianteBase)
                                                                    <div class="small text-primary fw-semibold">
                                                                        ...
                                                                        Variante:
                                                                        {{ $varianteBase->nombre ?? ($varianteBase->opcion?->etiqueta ?? 'Principal') }}
                                                                    </div>
                                                                @endif
                                                                ...
                                                                <div class="mt-2">
                                                                    @if ($tieneDescuento)
                                                                        <div
                                                                            ...
                                                                            ₡{{ number_format($precioNormalRelacionado, 0, ',', '.') }}
                                                                        </div>

                                                                        <div class="fw-bold text-danger">
                                                                            ₡{{ number_format($precioVentaRelacionado, 0, ',', '.') }}
                                                                        </div>
                                                                ...
                                                                <div
                                                                    class="small mt-1 {{ $stockRelacionado > 0 ? 'text-success' : 'text-danger' }}">
                                                                    <i
                                                                        class="bx {{ $stockRelacionado > 0 ? 'bx-check-circle' : 'bx-x-circle' }}"></i>
